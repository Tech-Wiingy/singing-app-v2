...
const SecondPage = ({ onNavigate, onDataUpdate }) => {
  ...
  const {
    ...
    volume: 0.7,
    onEnd: () => {
      console.log('Audio playback ended on page 2');
    }
  });
    ...
    if (isPlaying) {
      ...
      const animateCircle = () => {
        ...
        if (currentNoteIndex !== -1) {
          ...
          setHighestNote(prev => {
            const newHighest = !prev ? { note: currentNote, index: currentNoteIndex } : 
              (currentNoteIndex > prev.index ? { note: currentNote, index: currentNoteIndex } : prev);
            
            // Send highest note data to App
            if (onDataUpdate) {
              onDataUpdate(prevData => ({
                ...prevData,
                highestNoteFromPage2: newHighest.note
              }));
            }
        ...
        if (progress < 1) {
          ...
          setTimeout(() => {
            ...
            // Hide after 3 seconds and show glass overlay
            setTimeout(() => {
  ...
  // Function to get Y coordinate along the area contour (bottom-left to top-right)
  const getContourY = (x) => {
    // Follow the contour of the previous area from bottom-left to top-right
    if (x <= 100) return 280; // Start at bottom (C4 level)
    if (x <= 150) return 280 - ((x - 100) / 50) * 60; // Rise to 220 (D4 level)
    if (x <= 250) return 220; // Flat at D4 level
    if (x <= 300) return 220 - ((x - 250) / 50) * 60; // Rise to 160 (F4 level)
    if (x <= 400) return 160; // Flat at F4 level
    if (x <= 450) return 160 - ((x - 400) / 50) * 60; // Rise to 100 (G#4 level)
    if (x <= 550) return 100; // Flat at G#4 level
    if (x <= 600) return 100 - ((x - 550) / 50) * 60; // Rise to 40 (B4 level)
    return 40; // End at top (B4 level)
  };
  ...
  // Function to get Y coordinate for user's path (different contour)
  const getUserContourY = (x) => {
    // Different path for user interaction - B2, C3, E3 sequence
    if (x <= 200) return 260; // Start at B2 level  
    if (x <= 250) return 260 - ((x - 200) / 50) * 60; // Rise to C3 (200)
    if (x <= 450) return 200; // Flat at C3 level
    if (x <= 500) return 200 - ((x - 450) / 50) * 80; // Rise to E3 (120)
    return 120; // End at E3 level
  };

  return (
    <div className="empty-screen">
      <div className="empty-screen-content">
        <h1 className="empty-title">let's go higher</h1>
        <PlayControls
          onPlay={handlePlay}
          onPause={handlePause}
          isPlaying={isPlaying}
        ...
        {/* Minimalistic Musical Notes Graph */}
        <div style={{
          marginTop: '2.5rem',
          position: 'relative'
        }}>
            ...
            <div style={{
              ...
              {showStrikeThrough && (
                <div style={{
                  ...
                  <button
                    onClick={() => {
                      // Play Next logic - navigate to third page
                      setShouldExitSecondOverlay(true);
                      // Clean up after exit animation and navigate
                      setTimeout(() => {
                        setShowSecondOverlay(false);
                        setShouldExitSecondOverlay(false);
                        setShowStrikeThrough(false);
        ...
        <MicrophoneButton 
          ...
          onTap={() => {
            console.log('Mic tapped on Page 2');
            setMicActivated(true);
            // Keep mic button visible for voice input
          }}
          onVoiceDetected={() => {
            console.log('Voice detected on Page 2 - starting user path');
            if (micActivated) {
